    ...
    public sealed class InspectorProperty : IDisposable
    {
        ...
        public bool Update(bool forceUpdate = false)
        {
            ...
            if (this.ValueEntry != null)
            {
                ...
                else if (this.ValueEntry.SerializationBackend.IsUnity)
                {
                    var count = this.ParentValues.Count;
                    ...
                    if (this.ChildResolver.IsCollection)
                    {
                        if (this.cachedUnityPropertyListLengthModificationPath == null)
                        {
                            this.cachedUnityPropertyListLengthModificationPath = this.UnityPropertyPath + ".Array.size";
                        }

                        for (int i = 0; i < count; i++)
                        {
                            mod = this.Tree.PrefabModificationHandler.GetUnityPropertyModification(this.cachedUnityPropertyListLengthModificationPath, i, out childrenHaveModifications);

                            if (mod != null)
                            {
                                this.BaseValueEntry.ListLengthChangedFromPrefab = true;
                                break;
                            }
                        }
                    }
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        mod = this.Tree.PrefabModificationHandler.GetUnityPropertyModification(this.UnityPropertyPath, i, out childrenHaveModifications);

                        if (mod != null || (this.ValueEntry.IsMarkedAtomic && childrenHaveModifications))
                        {
                            this.BaseValueEntry.ValueChangedFromPrefab = true;
                            break;
                        }
                    }
                }
