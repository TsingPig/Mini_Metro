...
namespace Sirenix.OdinInspector.Editor
{
    ...
    public abstract class PropertyTree : IDisposable
    {
        ...
        public void BeginDraw(bool withUndo)
        {
            ...
            if (drawnInspectorDepthCount == 1 && Licensing.LicenseManager.LicensingIsEnabled && Licensing.LicenseManager.LicenseInfo.ClaimType == Licensing.ClaimedLicenseType.Disabled)
            {
                EditorGUILayout.Space();
                SirenixEditorGUI.ErrorMessageBox("You have chosen to disable Odin and not claim a seat license. In order to use tools and inspectors made with Odin, please activate Odin and claim a seat.");
                if (GUILayout.Button("Go to license management"))
                {
                    Licensing.OdinActivationWindow.Open();
                }
                EditorGUILayout.Space();
